...
export default function SettingsClient() {
  ...
  const handleExportOrdersOnly = () => {
    try {
      const orders = localStorage.getItem(KEY_OPEN_ORDERS) || '[]';
  ...
  const handleOrdersFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = async (e) => {
      try {
        ...
        localStorage.setItem(KEY_OPEN_ORDERS, JSON.stringify(data));
        
        if (db) {
            toast({ title: "Restaurando na Nuvem...", description: "Sincronizando mesas, por favor aguarde." });
            for (const order of data) {
                if (order.id) {
                    await setDoc(doc(db, 'open_orders', order.id), JSON.parse(JSON.stringify(order)), { merge: true });
                }
            }
        }
